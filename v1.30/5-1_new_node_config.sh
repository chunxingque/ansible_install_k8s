...
echo -e "#################### \033[42m ssh auth \033[0m ###############################################################"
ansible-playbook playbook/add_node.yaml --tags ssh_auth
echo -e "#################### \033[42m install chrony & time sync  \033[0m ############################################"
ansible-playbook playbook/add_node.yaml --tags chrony -i k8s_hosts
echo -e "############### \033[42m update hostname and /etc/hosts & Close firewalld and selinux and swap space \033[0m #"
echo -e "#################### \033[42m system config and modules-load \033[0m #########################################"
ansible-playbook playbook/add_node.yaml --tags system_config -i k8s_hosts
echo -e "#################### \033[42m install containerd \033[0m #####################################################"
ansible-playbook playbook/add_node.yaml --tags containerd_install -i k8s_hosts
echo -e "#################### \033[42m  host import image \033[0m #####################################################"
ansible-playbook playbook/add_node.yaml --tags import_image -i k8s_hosts
echo -e "#################### \033[42m reboot all hosts \033[0m #######################################################"
